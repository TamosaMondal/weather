...
/**
 * Applies all corporate-proxy-safe curl options to a handle.
 * Called by every curl request in this file.
 */
function curl_set_corporate_opts($ch, $timeout = 10) {
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_TIMEOUT, $timeout);
    curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, min(5, $timeout));
    curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);   // follow proxy redirects
    curl_setopt($ch, CURLOPT_MAXREDIRS, 5);
    curl_setopt($ch, CURLOPT_ENCODING, '');            // accept gzip/deflate
    curl_setopt($ch, CURLOPT_USERAGENT,
        defined('CURL_USER_AGENT') ? CURL_USER_AGENT : 'WeatherBookmarkDashboard/1.0');

    if (defined('SSL_VERIFY_PEER') && SSL_VERIFY_PEER === false) {
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, false);
    }

    // Route through corporate proxy if configured, otherwise let curl
    // pick up the system proxy from environment variables automatically.
    if (defined('CURL_PROXY') && CURL_PROXY !== '') {
...
function http_get_json($url, $timeout = 10) {
    $ch = curl_init($url);
    curl_set_corporate_opts($ch, $timeout);
