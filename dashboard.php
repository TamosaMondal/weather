...
<section class="cities-section">
    ...
    <?php else: ?>
        <!-- Filled in and shown/hidden by js/script.js based on the
             Notifications > Weather Alerts / Alert Threshold menu. -->
        <div id="weatherAlertsBanner" class="weather-alerts-banner hidden" role="alert"></div>

        <!-- Skeleton placeholders shown immediately, swapped out for the
             real grid (below) once the page has finished loading. See
             the ".content-loading" handling in js/script.js. -->
        <div class="city-grid skeleton-grid" aria-hidden="true">
            <?php for ($i = 0; $i < count($cities); $i++): ?>
                <div class="city-card skeleton-card">
                    <div class="city-card-header">
                        <span class="skeleton skeleton-icon"></span>
                        <div style="flex:1">
                            <div class="skeleton skeleton-text skeleton-line-medium"></div>
        ...
        <div class="city-grid content-loading" id="cityGrid">
            <?php foreach ($cities as $c): ?>
                <div class="city-card<?php echo $c['pinned'] ? ' pinned-card' : ''; ?>"
                     data-name="<?php echo h(strtolower($c['alias'] ?: $c['city_name'])); ?>"
                     data-pinned="<?php echo $c['pinned'] ? '1' : '0'; ?>"
                     data-updated="<?php echo h($c['updated_at']); ?>"
                     data-aqi-category="<?php echo $c['aqi'] !== null ? h($c['aqi_category']) : ''; ?>">
                    ...
                    <div class="city-actions">
                        ...
                        <form method="POST" action="actions/delete_city.php" class="inline-form delete-form">
                            ...
                            <button type="submit" class="btn btn-small btn-danger delete-confirm hidden">Sure?</button>
                            <button type="button" class="btn btn-small delete-cancel hidden">Cancel</button>
                        </form>
                    </div>
                </div>
            <?php endforeach; ?>
